...
// availabilityCache is imported from ../services/availabilityCache
// It is also used by adminController to invalidate on blocked-slot changes.
...
const assertSlotAvailable = async (candidate, ignoreId = null) => {
  ...
  const [allBookings, allBlockedSlots] = await Promise.all([
    bookings.list(),
    blockedSlots.list(),
  ]);
  const blocked = allBlockedSlots.find((item) => (
    item.eventDate === candidate.eventDate &&
    (!item.eventTime || item.eventTime === candidate.eventTime)
  ));
...
const getSlotAvailability = asyncHandler(async (req, res) => {
  ...
  // Serve from cache when possible to avoid repeated Firestore reads
  const cached = availabilityCache.get(eventDate);
  if (cached) {
    res.set("X-Availability-Cache", "HIT");
  ...
  const bookedSlots = data
    ...
    .filter(Boolean);
  const matchingBlocks = blocked.filter((item) => item.eventDate === eventDate);
  const blockedDate = matchingBlocks.some((item) => !item.eventTime);
  const blockedSlotsForDate = blockedDate
    ? DEFAULT_SLOT_TIMES
    : matchingBlocks.map((item) => item.eventTime).filter(Boolean);
  const allBookedSlots = [...new Set([...bookedSlots, ...blockedSlotsForDate])];
  ...
  res.set("X-Availability-Cache", "MISS");
...
const createBooking = asyncHandler(async (req, res) => {
  ...
  const booking = await withBookingWriteLock(async () => {
    ...
    availabilityCache.invalidate(); // Clear cache after write
    return result;
  });
...
const updateBookingStatus = asyncHandler(async (req, res) => {
  ...
  availabilityCache.invalidate(); // Clear cache after status change
  res.json({ success: true, data: booking });
});
...
const deleteBooking = asyncHandler(async (req, res) => {
  ...
  availabilityCache.invalidate(); // Clear cache after delete
