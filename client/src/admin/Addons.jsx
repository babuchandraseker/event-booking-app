...
/* ── Image upload helper — returns { url, fileId } ── */
  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = async () => {
      try {
        ...
        console.log('[Addon Upload] ImageKit upload starting…')
        ...
        console.log('[Addon Upload] Image URL:', result.url || result.path)
        console.log('[Addon Upload] fileId:', result.fileId || '(none)')
        resolve({ url: result.url || result.path, fileId: result.fileId || null })
      } catch (err) {
        reject(err)
      }
    }
...
async function savePackageAPI(pkg) {
  ...
  const payload = {
    ...
    // freeAddonNames must be persisted so "free" status survives server restarts
    freeAddonNames: Array.isArray(pkg.freeAddonNames) ? pkg.freeAddonNames : [],
    addons: (pkg.addons || []).map((a) => ({
      ...a,
      img: a.img || '',
...
export default function Addons() {
        ...
        if (!ignore) {
          data.forEach((pkg) => {
            (pkg.addons || []).forEach((a) => {
              console.log(`[Admin] Loaded addon image: pkg="${pkg.id}" addon="${a.name}" img="${a.img || '(empty)'}"`)
            })
          })
          setPackages(data.map(toForm))
        }
      })
      .catch(() => {
  ...
  async function handleAddonImageUpload(file, globalIndex) {
    ...
    setStatus({ msg: '', type: 'info' })
    try {
      const { url: imgUrl, fileId: imgFileId } = await uploadAddonImage(file, token)

      // Compute the next state for the ACTIVE package only, synchronously from
      // the current React state snapshot. Do NOT rely on the setState functional
      // updater's side-effect to capture the value — in React 18 async functions,
      // setState batches may be deferred past the next await.
      //
      // We intentionally do NOT touch any other package here. The backend's
      // syncAddonChanges() is solely responsible for propagating this addon's
      // new image to the same-named addon in the other packages — sending our
      // own concurrent PATCH for those packages is exactly the read-then-write
      // race that reverts fields, so we never do it from the client.
      const updatedActivePkg = (() => {
        const addons = activePkg.addons.map((a, i) => {
          if (i !== globalIndex) return a
          const updated = { ...a, img: imgUrl }
          if (imgFileId) updated.fileId = imgFileId
          return updated
        })
        return { ...activePkg, addons }
      })()

      // Commit optimistic UI update for the active package only.
      setPackages((prev) => prev.map((p) => (p.id === activePkg.id ? updatedActivePkg : p)))

      setStatus({ msg: '✓ Image uploaded — saving…', type: 'info' })

      // Save ONLY the package currently being edited. Exactly one PATCH request
      // is sent. The backend propagates the new img/fileId to the matching addon
      // in every other package via a concurrency-safe Firestore transaction.
      console.log('[Addon Upload] Firestore update started — saving active package only:', activePkg.id, 'img:', imgUrl)
      await savePackageAPI(updatedActivePkg)
      console.log('[Addon Upload] Firestore update finished')

      // Never trust optimistic local state after a save — reload everything
      // fresh from Firestore so the admin UI reflects exactly what was
      // persisted (including any addon fields propagated by the backend sync).
      const fresh = await fetchPackages()
      setPackages(fresh.map(toForm))

      // Only clear local preview AFTER the refreshed state lands so no flash.
  ...
  async function handleNewAddonImageUpload(file) {
    ...
    try {
      const { url: imgUrl, fileId: imgFileId } = await uploadAddonImage(file, token)
      setNewAddon((p) => ({ ...p, img: imgUrl, ...(imgFileId ? { fileId: imgFileId } : {}) }))
  ...
  async function savePackage() {
    ...
    try {
      // Save ONLY the package currently being edited — exactly one PATCH request.
      // Addon changes (name, price, desc, img) that need to reflect across
      // Silver, Gold and Platinum are propagated by the backend's
      // concurrency-safe syncAddonChanges(), not by the client PATCHing every
      // package itself. Sending a PATCH per package from here was the source
      // of the read-then-write race that reverted fields after refresh.
      await savePackageAPI(activePkg)

      // Never trust optimistic local state after saving. Reload every package
      // fresh from Firestore so the admin UI shows exactly what was persisted,
      // including any fields the backend sync propagated to other packages.
      const fresh = await fetchPackages()
      setPackages(fresh.map(toForm))

      setHasUnsaved(false)
      setStatus({ msg: `✓ Package saved — add-on changes are now live across all packages on the website.`, type: 'ok' })
      window.dispatchEvent(new CustomEvent('packagesUpdated'))
    } catch (err) {
      setStatus({
