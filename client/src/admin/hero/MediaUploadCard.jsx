...
export default function MediaUploadCard({
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
        <div style={{ minWidth: 0, flex: 1 }}>
          ...
          {isReplacing && !busy && (
            <div style={warningBox}>
              Uploading a new file will permanently replace the existing upload for this panel.
            </div>
          )}
          {limitReached && !isReplacing && !busy && (
            <div style={errorBox}>
              Upload limit reached. Delete or replace an existing video before adding another one.
            </div>
          )}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flexShrink: 0 }}>
          ...
          <button
            ...
            style={disabled || busy ? btnPrimaryDisabled : btnPrimary}
            title={limitReached ? 'Delete or replace an existing video first' : undefined}
          >
      ...
      >
        Drop file here or use {isReplacing ? 'Replace' : 'Upload'} - MP4 / WebM for video; JPG / PNG / WebP for poster
      </div>
    </AdminGlassCard>
  )
}
