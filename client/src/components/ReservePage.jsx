...
const TERMS_AND_CONDITIONS = [
  'We do not provide any movie/OTT accounts. Setups use your OTT accounts/downloaded content.',
  'Smoking/Drinking is not allowed inside the theater.',
  'Any damage to theater/decorative materials must be reimbursed.',
  'Guests must maintain cleanliness inside the theater.',
  'Party poppers/Snow sprays/Cold Fire and similar items are prohibited.',
  'Aadhaar Card is mandatory and will be scanned during entry.',
  'Pets are not allowed inside the theatre.',
  'Kids above 10 years are fully chargeable; ages 3-10 are half chargeable.',
  'Refund Policy: Advance is fully refundable if cancelled at least 72 hrs before slot. If slot is less than 72 hrs from payment time, advance is non-refundable.',
];
...
const bookingEmailJsConfig = {
  serviceId: import.meta.env.VITE_BOOKING_EMAILJS_SERVICE_ID,
  templateId: import.meta.env.VITE_BOOKING_EMAILJS_TEMPLATE_ID,
  publicKey: import.meta.env.VITE_BOOKING_EMAILJS_PUBLIC_KEY,
};
...
async function sendBookingConfirmationEmail(templateParams) {
  const enabled = bookingEmailJsConfig.serviceId && bookingEmailJsConfig.templateId && bookingEmailJsConfig.publicKey;
  ...
  const response = await fetch('https://api.emailjs.com/api/v1.0/email/send', {
    ...
    body: JSON.stringify({
      service_id: bookingEmailJsConfig.serviceId,
      template_id: bookingEmailJsConfig.templateId,
      user_id: bookingEmailJsConfig.publicKey,
...
async function fetchJsonWithTimeout(url, options = {}) {
  ...
  try {
    ...
    if (error.name === 'AbortError') {
      throw new Error('Booking request timed out. Please check the backend/Firebase connection and try again.', { cause: error });
    }
    throw error;
  } finally {
...
function getStoredBookingContext(themeKey) {
  try {
    ...
    const dateInput = parsed.dateInput || (parsed.date
      ? formatInputDate(new Date(parsed.date.year, parsed.date.month, parsed.date.day))
      : '');
    ...
    return {
      ...
      quickReserve: parsed.quickReserve === true,
...
function getEventDate(booking) {
  ...
  return formatInputDate(new Date(booking.date.year, booking.date.month, booking.date.day));
}
...
function DetailsForm({ booking, update, onNext }) {
  ...
  const isQuickReserve = booking.quickReserve === true;
  const hasSavedContact = isQuickReserve && Boolean(booking.phone && booking.whatsapp);
  const hasSavedDate = isQuickReserve && Boolean(getEventDate(booking));
  ...
  const quickReservedSlotUnavailable = Boolean(
    isQuickReserve &&
    booking.slot &&
    eventDate &&
    bookedTimes.includes(SLOT_MAP[booking.slot])
  );
  const hasSavedSlot = isQuickReserve && Boolean(booking.slot) && !quickReservedSlotUnavailable;
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE_URL}/bookings/availability/slots?date=${eventDate}`)
      ...
      .then(result => {
        ...
        if (!isQuickReserve && booking.slot && nextBookedTimes.includes(SLOT_MAP[booking.slot])) {
          update({ slot: '' });
    ...
    return () => { ignore = true; };
  }, [eventDate, booking.slot, update, isQuickReserve]);
  ...
  const handleChange = (key) => (event) => {
    ...
    if (key === 'slot' && errors.slot) setErrors(prev => ({ ...prev, slot: '' }));
  };
  ...
  const fields = [
    ...
    if (field.key === 'phone' && hasSavedContact) return false;
    if (field.key === 'whatsapp' && hasSavedContact) return false;
    if (field.key === 'dateInput' && hasSavedDate) return false;
    return true;
  });
  ...
  return (
    <motion.div className="reserve-details" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -24 }}>
      <div className="reserve-form-header">
        ...
        <h2 className="reserve-form-title">{isQuickReserve ? 'Complete Your Booking' : 'Booking Details'}</h2>
        <p className="reserve-form-subtitle">
          {isQuickReserve
            ? 'Your quick reserve details are saved. Add the remaining details to continue.'
            : 'Tell us who to celebrate and when to prepare the room.'}
        </p>
      </div>

      {(hasSavedContact || hasSavedDate || hasSavedSlot) && (
        <div className="reserve-quick-summary">
          {hasSavedDate && (
            <div>
              <span>Date</span>
              <strong>{formatDate(booking)}</strong>
            </div>
          )}
          {hasSavedSlot && (
            <div>
              <span>Slot</span>
              <strong>{SLOT_MAP[booking.slot] || '-'}</strong>
            </div>
          )}
          {booking.phone && (
            <div>
...
function TermsConfirm({ booking, themeInfo, totals, onConfirm, onBack }) {
  ...
  return (
    <motion.div className="reserve-terms" initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -24 }}>
      ...
      <div className="reserve-terms-panel" aria-labelledby="reserve-terms-heading">
        <div className="reserve-terms-panel-head">
          <span className="reserve-terms-kicker">Required before payment</span>
          <h3 id="reserve-terms-heading">Terms and Conditions</h3>
        </div>
        <div className="reserve-policies reserve-policies--terms">
          {TERMS_AND_CONDITIONS.map((term) => (
            <div key={term} className="reserve-policy-item">
              <span className="reserve-policy-icon">-</span>
              <span>{term}</span>
            </div>
          ))}
      ...
      <label className="reserve-agree-label">
        ...
        <span className="reserve-agree-text">I have read and accept all terms and conditions above. I understand the selected date and slot will be reserved only after payment.</span>
      </label>
...
function PaymentSummary({ booking, themeInfo, totals, onDone }) {
  ...
  const handleBook = async () => {
    ...
    try {
      ...
      const { response, result } = await fetchJsonWithTimeout(`${API_BASE_URL}/bookings`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          name: booking.name,
          ...
          guestCount: 1,
          location: booking.location || 'A WonderOne Suprise Studio',
          addons: addonNames,
      ...
      if (!response.ok || !result.success) {
        const message = result.message?.includes('RESOURCE_EXHAUSTED')
          ? 'Firebase quota is exceeded. Use local mode or increase Firebase quota, then try again.'
      ...
      try {
        await sendBookingConfirmationEmail({
          ...
          package_name: booking.packageName || PACKAGE_NAMES[booking.packageId] || themeInfo.name,
          event_date: getEventDate(booking),
          event_time: SLOT_MAP[booking.slot],
          payment_status: paymentMode === 'full' ? 'Paid in full' : 'Advance paid',
          paid_amount: `Rs ${amountToPay}`,
          total_amount: `Rs ${totals.total}`,
          remaining_amount: `Rs ${remaining}`,
          phone: booking.phone,
        });
      } catch (emailError) {
        console.warn('Booking confirmation email failed', emailError);
        emailNotice = 'Booking is confirmed, but the confirmation email could not be sent. Please check EmailJS settings.';
      }
  ...
  if (success) {
    const whatsappText = encodeURIComponent(`Hi A WonderOne Suprise, my booking ${success.id} is confirmed. I need support.`);
    ...
    return (
      <motion.div className="reserve-success" initial={{ opacity: 0, scale: 0.92 }} animate={{ opacity: 1, scale: 1 }} transition={{ type: 'spring', damping: 22, stiffness: 260 }}>
        ...
        <p className="reserve-success-sub">Your booking has been confirmed. We'll contact you shortly to process the advance payment.</p>
        {success.emailNotice && <p className="reserve-error">{success.emailNotice}</p>}
...
export default function ReservePage() {
  ...
  // Always start at the very top of the page on mount
  useEffect(() => {
  ...
  const goToPhase = useCallback((nextPhase) => {
    setPhase(nextPhase);
    // On mobile (stacked layout) scroll to top of main content on step change
    requestAnimationFrame(() => {
      if (mainRef.current) {
        const top = mainRef.current.getBoundingClientRect().top + window.scrollY - 16;
  ...
  return (
    <div className="reserve-page">
      ...
      <header className="reserve-nav">
        ...
        <span className="reserve-nav-brand">A WonderOne Suprise</span>
      ...
      <div className="reserve-layout">
        ...
        <main className="reserve-main" ref={mainRef}>
          <AnimatePresence mode="wait">
            {phase === 'details' && <DetailsForm key="details" booking={booking} update={update} onNext={() => goToPhase('terms')} />}
            {phase === 'terms' && <TermsConfirm key="terms" booking={booking} themeInfo={themeInfo} totals={totals} onConfirm={() => goToPhase('payment')} onBack={() => goToPhase('details')} />}
