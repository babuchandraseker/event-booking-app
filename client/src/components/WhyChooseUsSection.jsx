...
export default function WhyChooseUsSection() {
  ...
  return (
    <section
      ref={ref}
      id="why-us"
      className="relative border-y border-[rgba(201,168,76,0.1)] bg-gradient-to-b from-vn-deep to-vn-black py-20 sm:py-28"
        ...
        >
          {pillars.map((p) => (
            <motion.article
              key={p.title}
              variants={card}
              className="group relative overflow-hidden rounded-2xl border border-[rgba(201,168,76,0.12)] bg-[rgba(15,15,26,0.55)] p-7 shadow-[0_20px_60px_rgba(0,0,0,0.4)] backdrop-blur-md transition-all duration-300 ease-luxury hover:border-[rgba(201,168,76,0.3)] hover:shadow-glow-gold sm:p-8"
            >
              <div
                className="pointer-events-none absolute -right-10 -top-10 h-32 w-32 rounded-full bg-[rgba(201,168,76,0.07)] blur-2xl transition-opacity duration-500 group-hover:opacity-100"
                aria-hidden
              />
