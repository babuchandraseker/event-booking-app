...
/**
 * Fetch the live video URL for a theme from the API.
 * Returns scrollyMedia.video (Cloudinary) > theme.videoSrc > THEME_DATA static fallback.
 *
 * Usage:
 *   const videoUrl = await resolveThemeVideoUrl(apiBaseUrl, 'romantic');
 */
export async function resolveThemeVideoUrl(apiBaseUrl, themeKey) {
  try {
    const res = await fetch(`${apiBaseUrl}/themes?_=${Date.now()}`);
    if (!res.ok) throw new Error('API error');
    const json = await res.json();
    const theme = (json?.data || []).find((t) => t.id === themeKey || t.key === themeKey);
    if (!theme) throw new Error('Theme not found');
    return theme.scrollyMedia?.video || (theme.videoSrc?.startsWith('http') ? theme.videoSrc : null) || null;
  } catch {
    return null;
  }
}
